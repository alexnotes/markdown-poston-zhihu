...
GITHUB_REPO_PREFIX = "https://raw.githubusercontent.com/alexnotes/markdown-poston-zhihu/master/Data/"
...
def rename_image_ref(m, original=True):
    ...
    if os.path.getsize(image_folder_path.parent/m.group(1+int(original))) > COMPRESS_THRESHOLD:
        ...
        if original:
            ...
            image_ref_name = Path(m.group(1)).name
    if original:
        return "!["+m.group(1)+"]("+GITHUB_REPO_PREFIX+str(image_folder_path.name)+"/"+quote(image_ref_name)+")"
    else:
        return '<img src="'+GITHUB_REPO_PREFIX+str(image_folder_path.name)+"/" + quote(image_ref_name) + '"'
...
def reduce_image_size():
    ...
    for image_path in [i for i in list(image_folder_path.iterdir()) if not i.name.startswith(".") and i.is_file()]:
        ...
        if os.path.getsize(image_path) > COMPRESS_THRESHOLD:
            ...
            img.convert('RGB').save(str(image_folder_new_path /
                                        (image_path.stem+".jpg")), optimize=True, quality=85)
        else:
